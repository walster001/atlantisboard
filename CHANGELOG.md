...
### Edge Functions
- `generate-invite-token`: Create board invite tokens
- `redeem-invite-token`: Process invite link redemptions
- `verify-user-email`: Check user verification status against external database
- `import-wekan-board`: Handle Wekan board imports
- `save-mysql-config`: Store MySQL connection configuration
- `test-mysql-connection`: Validate MySQL connectivity

### Database Tables
- `workspaces`: Workspace management
- `boards`: Board storage with theme and background settings
- `columns`: Kanban columns
- `cards`: Card data with descriptions, due dates, priorities
- `board_members`: Role-based board membership
- `board_member_audit_log`: Member activity tracking
- `board_invite_tokens`: Invite link management
- `board_themes`: Theme configurations
- `labels` / `card_labels`: Card labeling system
- `card_subtasks`: Checklist items
- `card_assignees`: User assignments to cards
- `card_attachments`: File attachments
- `profiles`: User profiles with admin flag
- `app_settings`: Global application settings
- `custom_fonts`: Custom font storage
- `mysql_config`: External database configuration
- `import_pending_assignees`: Import user mapping
- `import_pending_attachments`: Import file tracking

---

## How to Update This Changelog

When making changes, add entries under the appropriate date heading with these categories:
- **Added**: New features
