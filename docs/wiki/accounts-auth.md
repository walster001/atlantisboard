...
If the administrator has enabled **mandatory email verification** in [Login Options](admin-login-options.md), a "check your email" screen appears immediately after successful registration. You must click the verification link in the email before you can sign in.
...
- Verified status is visible to administrators in the [User Management](admin-users.md) panel.
...
Depending on the [authentication method](admin-login-options.md) configured by the administrator, the login page may also display a **Sign in with Google** button. See [Google OAuth Sign-In](google-oauth.md) for details.

---

## Registration Modes

The administrator controls how new accounts are created via the **Registration Mode** setting in [Login Options](admin-login-options.md):
...
- The user cannot sign in until an administrator unlocks their account from the [User Management](admin-users.md) panel.
...
- [Login Options](admin-login-options.md) — configure authentication methods and registration modes.
- [Password Reset & Email Verification](password-reset.md) — recover access if you forget your password.
- [Google OAuth Sign-In](google-oauth.md) — sign in using your Google account.
- [Creating the First Admin Account](first-admin-account.md) — first-time setup guidance.
- [User Management](admin-users.md) — administrator tools for managing user accounts.
