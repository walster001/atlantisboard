...
|---------|------|-------------|
| **Use custom favicon** | Checkbox | When checked, the navbar icon inherits the favicon uploaded in [Login Branding](admin-login-branding.md). This is a quick way to share a single icon across the login page and the app. |
...
| Setting | Type | Description |
|---------|------|-------------|
| **Inherit text from custom app name** | Checkbox | When checked, the label automatically uses the application name configured in [Login Branding](admin-login-branding.md). |
...
| **Custom fonts** | Any fonts you have uploaded via the [Custom Fonts](admin-custom-fonts.md) panel appear here as additional options. |

The selected font applies to all UI text throughout the application (navigation, buttons, labels, form fields, etc.). Individual branding elements (login app name, tagline) have their own font overrides configured in [Login Branding](admin-login-branding.md).
...
- [Login Branding](admin-login-branding.md) — customise the login and registration pages.
- [Email Branding](admin-email-branding.md) — customise outgoing email templates.
- [Custom Fonts](admin-custom-fonts.md) — upload fonts available in the UI font selector.
