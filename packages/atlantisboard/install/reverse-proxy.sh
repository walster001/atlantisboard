...
_prompt_proxy_field() {
  ...
  while [[ "$valid" != true ]]; do
    ...
    if atl_validate_value "$current" "$vtype" "$optional"; then
      ...
      atl_whiptail_msgbox --title "Invalid input" --msgbox "${label}\n\n${err_msg}" 12 70 || true
    fi
  done
}
...
_prompt_proxy_fields_for() {
  ...
  if [[ ! -f "$REVERSE_PROXY_FIELDS" ]] || ! command -v jq >/dev/null 2>&1; then
    atl_whiptail_msgbox --title "Reverse proxy" --msgbox \
      "jq or reverse-proxy-fields.json missing; cannot prompt proxy settings." 8 60
    return 1
  fi
  ...
  if [[ -z "$domain" ]]; then
    atl_whiptail_msgbox --title "Reverse proxy" --msgbox "Domain name is required for reverse proxy setup." 8 60
    return 1
  fi
...
_install_proxy_packages() {
  ...
  atl_whiptail_infobox --title "Installing packages" --infobox "Installing: ${pkgs[*]} ..." 8 70
...
_configure_nginx() {
  ...
  if [[ ! -f "$ssl_cert" ]] || [[ ! -f "$ssl_key" ]]; then
    ...
    atl_whiptail_msgbox --title "Nginx TLS" --msgbox \
  ...
  if ! atl_sudo nginx -t 2>/tmp/nginx-test.err; then
    atl_whiptail_msgbox --title "Nginx config error" --msgbox "$(cat /tmp/nginx-test.err)" 16 70
    return 1
  fi
  ...
  if [[ "$use_https_tpl" == false ]]; then
    ...
    if ! command -v certbot >/dev/null 2>&1; then
      if atl_whiptail_yesno --title "certbot" --yesno "Install certbot and python3-certbot-nginx?" 10 70; then
...
_configure_caddy() {
  ...
  atl_sudo chown -R caddy:caddy /var/log/caddy 2>/dev/null || atl_sudo chown -R root:root /var/log/caddy

  if [[ -n "${PROXY_VALUES[PROXY_ACME_EMAIL]:-}" ]]; then
    printf '%s\n' "{" "    email ${PROXY_VALUES[PROXY_ACME_EMAIL]}" "}" | atl_sudo tee "${conf_d}/00-acme-email.caddy" >/dev/null
  fi
  ...
  if atl_sudo test -f "$main_file" && ! grep -q 'conf.d/\*\.caddy' "$main_file" 2>/dev/null; then
  ...
  if command -v caddy >/dev/null 2>&1; then
    atl_sudo caddy validate --config "$main_file" || {
      atl_whiptail_msgbox --title "Caddy config error" --msgbox "caddy validate failed for ${main_file}" 8 60
      return 1
    }
  fi
