...
![Backup panel](/assets/wiki/admin-backup.png)
...
> **Docker users:** Ensure the backup path is mapped to a Docker volume or bind mount so backups persist outside the container. See [Environment Variables Reference](/wiki/environment-variables/) for details.
...
![Backup history list](/assets/wiki/admin-backup-history.png)
...
![Restore confirmation](/assets/wiki/admin-backup-restore.png)
...
- **Set a backup location early** — configure `BACKUP_LOCATION` during [initial setup](/wiki/initial-configuration/) before you start creating boards.
...
- **Monitor disk space** — backup archives can grow large, especially with many card attachments. Use the [System Monitor](/wiki/admin-monitor/) to track disk usage.
...
- [Environment Variables Reference](/wiki/environment-variables/) — `BACKUP_LOCATION`, `BACKUP_MC_PATH`, and `BACKUP_MC_MIRROR_ALIAS` settings.
- [Database Maintenance](/wiki/admin-database/) — clean up orphaned data before creating a backup.
- [System Monitor](/wiki/admin-monitor/) — monitor disk usage and system health.
- [Updating & Maintenance](/wiki/updating/) — always back up before upgrading.
