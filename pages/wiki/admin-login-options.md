...
The Login Options panel controls how users authenticate with Atlantisboard and how new accounts are created. This is one of the first settings you should configure after creating your [first admin account](/wiki/first-admin-account/).
...
![Login options panel](/assets/wiki/admin-login-options.png)
...
![Registration mode selector](/assets/wiki/admin-registration-mode.png)
...
> **Note:** Regardless of this setting, the [first-user bypass](/wiki/first-admin-account/) always allows registration when the database has zero users.
...
![Google OAuth configuration](/assets/wiki/admin-google-oauth.png)
...
See the [Environment Variables Reference](/wiki/environment-variables/) for details.
...
- [First Admin Account](/wiki/first-admin-account/) — how the first user is created and promoted.
- [Initial Configuration Walkthrough](/wiki/initial-configuration/) — where Login Options fits in the setup process.
- [Environment Variables Reference](/wiki/environment-variables/) — Google OAuth and MySQL environment variables.
- [Permissions & Roles](/wiki/admin-permissions/) — controlling what users can do after they sign in.
