...
![Export options](/assets/wiki/export-options.png)
...
> **Note:** The 25 MB embedded attachment limit is a per-export cap. If your board's total attachment size exceeds this, larger attachments are excluded from the export with a reference note. For full backup including all attachments regardless of size, use the [Backup & Restore](/wiki/admin-backup/) system instead.
...
- [Importing Boards](/wiki/import/) — Import boards from the same formats.
- [Backup & Restore](/wiki/admin-backup/) — Full system backup including all attachments without size limits.
- [Permissions & Roles](/wiki/admin-permissions/) — Configure export permissions per role.
