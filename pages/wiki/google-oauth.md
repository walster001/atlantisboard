...
> **Prerequisite:** Google OAuth must be enabled and configured by an administrator in [Login Options](/wiki/admin-login-options/). This requires a Google Cloud Console project with OAuth 2.0 credentials.

---

## How Google Sign-In Works

![Google sign-in button](/assets/wiki/auth-google.png)
...
If this is your first sign-in and no local account exists with your Google email, a new account is created automatically (subject to the configured [registration mode](/wiki/accounts-auth/#registration-modes)).
...
If this query returns at least one row, verification passes. The query is executed against the external MySQL database configured in the [Login Options](/wiki/admin-login-options/) panel.
...
These parameters are passed to Google during the OAuth flow to enable authentication on private networks. See [Environment Variables Reference](/wiki/environment-variables/) for the full list of Google OAuth variables.
...
5. Copy the **Client ID** and **Client Secret** into the [Login Options](/wiki/admin-login-options/) panel in Atlantisboard.
6. Select an authentication method that includes Google (e.g. "Local Accounts + Google" or "Google Login Only").
7. Save the configuration.

For detailed field descriptions and the "Replace credentials" workflow, see [Login Options](/wiki/admin-login-options/).

---

## Related Pages

- [Login Options](/wiki/admin-login-options/) — configure Google OAuth credentials and authentication methods.
- [Registration & Sign-In](/wiki/accounts-auth/) — registration modes and local account sign-in.
- [Environment Variables Reference](/wiki/environment-variables/) — Google OAuth and LAN-related environment variables.
- [Password Reset & Email Verification](/wiki/password-reset/) — email verification for merged accounts.
