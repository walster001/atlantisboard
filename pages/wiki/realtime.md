...
**Oplog sizing:** change streams depend on the replica set oplog. Size it to retain at least **24–48 hours** of writes so a deploy or outage does not roll the oplog past your last resume token. See [Docker Compose Installation](/wiki/docker-compose-install/#mongodb--database) and [Deployment overview](../../DEPLOYMENT.md#mongodb-oplog-sizing-change-streams).
...
If the browser or server is unreachable, Atlantisboard shows a persistent **“Offline mode”** notification (yellow banner) with the message that you are offline and changes will not be saved. That notice clears automatically when connectivity returns. See [Offline & PWA](/wiki/offline-pwa/) for queued changes and cached boards.
...
- **MongoDB must be configured as a replica set** — Change Streams require a replica set (or sharded cluster). A standalone MongoDB instance does not support Change Streams. See [Docker Compose Installation](/wiki/docker-compose-install/) or [Manual Installation](/wiki/manual-install/) for setup instructions.
...
- [Board Overview](/wiki/board-overview/) — the board interface where real-time updates are most visible.
- [Card Detail](/wiki/card-detail/) — comments, checklists, and other card data synced in real time.
- [Environment Variables Reference](/wiki/environment-variables/) — configuring change streams and connection settings.
- [Offline & PWA](/wiki/offline-pwa/) — what happens when the connection is lost.
