...
> **Warning:** Re-running setup **regenerates secrets in `.env`** while old volumes may still hold previous passwords. If MongoDB or Redis fail auth after an upgrade, either restore the old `.env` secrets or reset data — see [Debian installation — troubleshooting](/wiki/debian-install/#mongodb-unhealthy--scram-authentication-failed-storedkey-mismatch).
...
- [Debian installation (auto setup)](/wiki/debian-install/)
- [npm install (`atlantisboard`)](/wiki/npm-install/)
