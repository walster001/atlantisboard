...
![Change password form](/assets/wiki/user-change-password.png)
...
- Only an **App Admin** can unlock a locked account. This is done from the [User Management](/wiki/admin-users/) panel using the **Unlock** action.
- When an administrator unlocks the account, the failed attempt counter is reset to zero.
- The user can then sign in with their correct password (or reset it via [Password Reset](/wiki/password-reset/) if forgotten).
...
The theme setting affects the overall application chrome (navigation bars, sidebars, modals, dropdowns). Board-specific theming (colours, backgrounds) is controlled separately via [Board Settings](/wiki/board-settings-theme/).

---

## Related Pages

- [Profile Settings](/wiki/user-profile/) — update your display name, avatar, and language.
- [Registration & Sign-In](/wiki/accounts-auth/) — password requirements during registration.
- [Password Reset & Email Verification](/wiki/password-reset/) — recover access with a forgotten password.
- [User Management](/wiki/admin-users/) — administrators can unlock locked accounts.
- [Notification Preferences](/wiki/user-notifications/) — configure notification delivery channels.
