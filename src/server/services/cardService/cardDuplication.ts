...
async function loadTargetListContext(
  targetListId: string,
  userId: string,
...
async function loadListOrderAfterInsert(
  targetListId: string,
...
async function persistDuplicatedCards(
  targetListId: string,
...
function logDuplicationAuditAndActivities(args: {
  readonly created: readonly (Document & ICard)[];
  readonly sourceCards: readonly SourceCardForDuplicate[];
  readonly targetListId: string;
  readonly targetBoardId: string;
