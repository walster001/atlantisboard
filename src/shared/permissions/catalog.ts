...
export const BUILTIN_ROLE_HIERARCHY_LEVELS: Readonly<Record<BuiltInRoleKey, number>> = {
  viewer: 100,
  manager: 200,
  admin: 300,
} as const;
...
export const MEMBERS_ROLE_UPDATE_MODE_OPTIONS: ReadonlyArray<{ value: BoardMemberRoleUpdateModeKey; label: string }> = [
...
export const MEMBERS_ROLE_UPDATE_MODE_KEYS = new Set<string>(
  MEMBERS_ROLE_UPDATE_MODE_OPTIONS.map((option) => option.value),
);

/** Permission key used when granting or revoking App Admin in member management UIs. */
export const APP_ADMIN_ASSIGN_PERMISSION_KEY = 'app.roles.assign_app_admin' as const;
...
export const PERMISSION_DESCRIPTIONS: Readonly<Record<string, string>> = {
  ...
  'boards.settings.activitylog': 'View the board Activity Log in Board Settings.',
  ...
  'import.atlantisboard': 'Start an Atlantisboard import.',
...
/** Board-scoped permission keys requested by the client board page and Kanban chrome. */
...
/** Select options for built-in roles in member-management surfaces. */
export function builtinRoleSelectOptions(): ReadonlyArray<{ value: BuiltInRoleKey; label: string }> {
  return BUILTIN_ROLE_SEEDS.map((seed) => ({
    value: seed.key,
    label: seed.displayName,
  }));
}

export function permissionsForBuiltinRole(roleKey: BuiltInRoleKey): readonly string[] {
